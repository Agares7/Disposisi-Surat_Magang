                ...
                <?php
                    ...
                    $cari = mysqli_real_escape_string($config, $_REQUEST['cari']);
                        ...
                        <table class="bordered" id="tbl">
                            <thead class="green darken-1" id="head">
                                <tr>
                                    <th width="5%">No.</th>
                                    <th width="25%">Isi Ringkas<br/> File</th>
                            ...
                            if($query==true && mysqli_num_rows($query) > 0){
                                ...
                                while($row = mysqli_fetch_array($query)){
                                  echo '
                                    ...
                                    <td>';
                                    if ($query==true && mysqli_num_rows($query) > 0) {
                                        while($row99 = mysqli_fetch_array($query)){
                                            $query99 = mysqli_query($config, "SELECT * FROM tbl_disposisi");
                                            $count_disp99=mysqli_num_rows($query99);
    
                                            if($row['id_surat'] == $row99['id_surat']){
                                                echo '<em>Tidak ada file yang di upload</em>';
                                            } else {
                                                echo '<em>Tidak ada file yang di upload</em>';
                                            }
                                    ...
                                    while($row = mysqli_fetch_array($query)){
                                      ...
                                      echo '
                                        ...
                                        if($_SESSION['id_user'] != $row['id_user'] AND $_SESSION['id_user'] != 1 AND $_SESSION['admin']!=3){
                                            ...
                                            echo '<a class="btn small deep-green waves-effect waves-light" href="?page=tsm&act=del&id_surat='.$row['id_surat_masuk'].'">
                                            <i class="material-icons">delete</i> DEL</a>';
